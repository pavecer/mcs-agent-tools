...
def admin_management_panel() -> rx.Component:
    """Simple admin UI for DB-backed user management."""
    return card(
        rx.vstack(
            rx.hstack(
                rx.icon("users", color=PRIMARY, size=20),
                rx.heading("User Management", size="4", color="#201f1e"),
                spacing="2",
                align="center",
            ),
            rx.text(
                "Add and remove users stored in PostgreSQL. If password is left empty, a temporary password is generated and emailed when ACS is configured.",
                font_size="13px",
                color="#605e5c",
            ),
            rx.cond(
                State.admin_error != "",
                rx.callout(State.admin_error, icon="circle-alert", color_scheme="red", size="1"),
                rx.box(),
            ),
            rx.cond(
                State.admin_message != "",
                rx.callout(State.admin_message, icon="circle-check", color_scheme="green", size="1"),
                rx.box(),
            ),
            rx.hstack(
                rx.input(
                    placeholder="user@company.com",
                    value=State.add_user_email,
                    on_change=State.set_add_user_email,
                    width="50%",
                ),
                rx.input(
                    placeholder="Password (optional)",
                    type="password",
                    value=State.add_user_password,
                    on_change=State.set_add_user_password,
                    width="50%",
                ),
                spacing="2",
                width="100%",
            ...
            rx.vstack(
                rx.foreach(
                    ...
                    lambda user: rx.hstack(
                        rx.vstack(
                            rx.text(user["email"], font_size="13px", font_weight="600", color="#201f1e"),
                            rx.badge(
                        ...
                        rx.button(
                            "Delete",
                            on_click=State.delete_user_account(user["email"]),
                            variant="ghost",
                            color_scheme="red",
                            cursor="pointer",
                        ),
...
def reset_password_form() -> rx.Component:
    """First-login password reset form for DB-backed users."""
    return rx.center(
        card(
            rx.form.root(
                rx.vstack(
                    rx.hstack(
                        rx.icon("key-round", color=PRIMARY, size=26),
                        rx.heading("Set A New Password", size="5", color="#201f1e", font_weight="700"),
                        spacing="3",
                        align="center",
                    ),
                    rx.text(
                ...
                on_submit=State.submit_password_reset,
            ),
        ),
        min_height="100vh",
        background_color=BG,
                    ...
                    rx.vstack(
                        rx.cond(
                            ...
                            rx.cond(
                                ...
                                rx.vstack(
                                    rx.cond(
                                        State.mcs_analyse_tab == "model_comparison",
                                        card(
                                            rx.vstack(
                                                rx.hstack(
                                                    rx.button(
                                                        rx.hstack(
                                                            rx.cond(
                                                                State.mcs_api_comparison_running,
                                                                rx.spinner(size="1"),
                                                                rx.icon("zap", size=14),
                                                            ),
                                                            rx.text(
                                                        ...
                                                        on_click=State.run_mcs_api_comparison,
                                                        is_disabled=State.mcs_api_comparison_running,
                                                        color_scheme="blue",
                                                        variant="solid",
                                                        size="1",
                                                        cursor="pointer",
                                                    ...
                                                    rx.cond(
                                                        ...
                                                        rx.hstack(
                                                            rx.spinner(size="1"),
                                                            rx.text(
                                                                "Running live API comparison (this can take up to 30 seconds)",
                                                                font_size="12px",
                                                                color="#1f4e79",
                                                            ),
                                                            spacing="2",
                                                        ...
                                                        rx.cond(
                                                            State.can_run_mcs_api_comparison,
                                                            rx.text(
                                                                "Ready to run live API comparison",
                                                                font_size="12px",
                                                                color="#51627a",
                                                            ),
                                                            rx.cond(
                                                                State.mcs_env_api_key_present,
                                                                rx.text(
                                                                    "Live comparison will be enabled after snapshot context is available.",
                                                                    font_size="12px",
                                                                    color="#8a6d3b",
                                                                ),
                                                                rx.text(
                                                                    "Set OPENAI_API_KEY in .env and restart the app to enable live comparison.",
                                                                    font_size="12px",
                                                                    color="#8a6d3b",
                                                                ),
                                                            ),
                                            ...
                                            background="#f8fbff",
                                            border="1px solid #dbe6f6",
                                            box_shadow="0 6px 18px rgba(12, 33, 70, 0.05)",
                                        ),
                                        rx.box(),
                                    ),
                                    rx.foreach(State.mcs_current_section_segments, _mcs_segment_block),
