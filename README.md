...
### 8. DB-backed Login + User Management (optional)

The login experience supports DB-backed accounts with admin user management:

- admin signs in with `AUTH_ADMIN_EMAIL` + `AUTH_ADMIN_PASSWORD`
- users are stored in PostgreSQL (`auth_users`)
- admin can add and deactivate users in `/admin`
- optional autogenerated passwords can be emailed through Azure Communication Services

Required environment variables:

- `AUTH_DB_DSN=postgresql://...`
- `AUTH_ADMIN_EMAIL=admin@contoso.com`
- `AUTH_ADMIN_PASSWORD=...`
...
- `OPENAI_API_KEY`: optional key enabling LLM eval generation and live model comparison
- `OPENAI_BASE_URL`: canonical OpenAI endpoint override (Azure OpenAI or compatible endpoints)
- `OPENAI_API_BASE`: deprecated alias for `OPENAI_BASE_URL` (still supported for compatibility)
- `OPENAI_MODEL`: optional model override
- `MCS_ENABLE_MODEL_COMPARISON`: enable live comparison UI action
- `MCS_COMPARISON_MODE`: `simulated` (default) or `real`
- `MCS_COMPARISON_MODELS`: optional comma-separated comparison models
...
- `MCS_DATAVERSE_TOKEN`: bearer token for Dataverse API access (token mode)
- `MCS_AAD_TENANT_ID`: Entra tenant ID (service principal mode)
- `MCS_AAD_CLIENT_ID`: Entra app client ID (service principal mode)
- `MCS_AAD_CLIENT_SECRET`: Entra app client secret (service principal mode)
...
- `MCS_ADMIN_API_TOKEN`: optional bearer token for admin analytics API (falls back to `MCS_DATAVERSE_TOKEN`)
